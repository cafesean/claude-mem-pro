...
function checkMarketplaceMarker(): void {
  try {
    ...
    const marketplaceCandidates = [
      resolve(home, '.claude', 'plugins', 'marketplaces', 'cafesean'),
      resolve(home, '.config', 'claude', 'plugins', 'marketplaces', 'cafesean'),
    ];
    const present = marketplaceCandidates.some(p => p && existsSync(p));
    const cacheCandidates = [
      resolve(home, '.claude', 'plugins', 'cache', 'cafesean', 'claude-mem'),
      resolve(home, '.config', 'claude', 'plugins', 'cache', 'cafesean', 'claude-mem'),
    ];
